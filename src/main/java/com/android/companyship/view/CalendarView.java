package com.android.companyship.view;
...
import com.android.companyship.activity.CalendarActivityDialog;
...
public class CalendarView extends View implements View.OnTouchListener {
    ...
    public int[] date = new int[42]; // 日历显示数字
    public int curStartIndex, curEndIndex,curmidderIndex; // 当前显示的日历起始的索引
    ...
    public CalendarActivityDialog dialog;
...
//		canvas.drawRect(surface.weekHeight, surface.width, surface.weekHeight, surface.width, surface.cellBgPaint);
        ...
        if (curYearAndMonth.equals(todayYearAndMonth)) {
            int todayNumber = (calendar.get(Calendar.DAY_OF_MONTH)+ 1);
        ...
        String[] dateindex = dialog.data;
        for (int n =0 ;n < dateindex.length;n++){
            for (int j = 0;j<listdate.size();j++){
                if (dateindex[n].equals(listdate.get(j))){
                    color = surface.borderColor;
                    drawCellText(canvas, j, date[j] + "", color);
                }
            }
        }
        // 清空集合 重新加载数据  否则数据会重复
    ...
    private void calculateDate() {
        ...
        if (monthStart > 0) {
            ...
            for (int i = monthStart - 1; i >= 0; i--) {
                ...
                dayInmonth--;
                //把上一个月添加到集合里面
                listdate.add(calendar.get(Calendar.YEAR) + ""
                        + (calendar.get(Calendar.MONTH) + 1) + ""+ date[i] );
            }
        ...
        curmidderIndex = monthStart + calendarday.get(Calendar.DAY_OF_MONTH);
        for (int i = 0; i < monthDay; i++) {
            date[monthStart + i] = i + 1;
            //把这一个月添加到集合里面
            listdate.add(calendar.get(Calendar.YEAR) + ""
                    + (calendar.get(Calendar.MONTH) + 1) + ""+ date[monthStart + i] );
        }
        ...
        for (int i = monthStart + monthDay; i < 42; i++) {
            ...
            //把下一个月添加到集合里面
            listdate.add(calendar.get(Calendar.YEAR) + ""
                    + (calendar.get(Calendar.MONTH) + 2) + ""+ date[i] );
        }
        if (curEndIndex < 42) {
            // 显示了下一月的
    ...
    public boolean isMidderMonth(int i) {
        if (i < curmidderIndex) {
            return true;
        }
        return false;
...
//		}
    ...
    public class Surface {
        ...
        public void init() {
            ...
            float tempw = height / 7f;
            cellWidth = (width - (tempw * 1.1f)) / 7f;
            //画边框
